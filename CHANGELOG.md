...
- Least-privilege participant read receipts for explicit 1–100-subject room queries. A new room-scoped `room:read-receipts` permission gates both snapshots and live `read.updated` delivery; results preserve caller order and expose the exact message `(created_at, id)` cursor plus advancement time without enumerating membership or storing per-message receipt rows. PostgreSQL cursor/event updates are atomic, SQLite broadcasts only changed state, and unpublished TypeScript SDK 0.13.0 adds bounded queries and typed events.
- Backward-compatible `snapshot_sync_v1` WebSocket negotiation and an explicit `sync.completed` local catch-up boundary. Unpublished TypeScript SDK 0.12.0 uses it when advertised, retains ping/pong activation against older servers, and adds an in-memory `RoomTimeline` that replaces reconnect history, reconciles complete message mutations by ID, preserves stale state during outages, reports synchronization generations, and deterministically bounds retained messages (default 1000, configurable 1-10000) with truncation/cursor metadata.
- Bounded host-resolved message mentions across HTTP/WebSocket creation, author/admin edits, SQLite, PostgreSQL, history, replies, search, pins, export schema 8, realtime, selected signed webhooks, and unpublished TypeScript SDK 0.11.0. At most ten ordered unique stable subjects are stored; Samsarix does not parse display text, validate host membership, register devices, or deliver push/email notifications.
- SQLite schema v11 and PostgreSQL schema v14 add default-empty mention arrays. Tombstones clear targets and scrub them from retained PostgreSQL message events and prior webhook bodies; already delivered receiver copies remain outside the engine's control.
- A bounded `POST /v1/read-states/query` inbox primitive for 1–100 explicit, token-authorized room IDs. It returns input-ordered cursor/unread state, newest visible message ID/time, total unread messages, and unread-room count without message bodies or server-owned membership discovery. SQLite and PostgreSQL execute one statement per request; a dedicated per-subject allowance limits amplification. Unpublished TypeScript SDK 0.10.0 adds validated `queryReadStates()` support.
- Bounded host-owned attachment references across HTTP/WebSocket creation, attachment-only messages, SQLite, PostgreSQL, history, replies, search, pins, export schema 7, realtime, selected signed webhooks, and unpublished TypeScript SDK 0.9.0. Samsarix stores at most five opaque descriptors and deliberately does not accept blobs or signed URLs; host applications retain upload validation/scanning, access control, short-lived download issuance, object cleanup, and storage/egress cost.
...
- A fifth measured `reconnect-storm` profile pauses one real replica while writes continue, freezes/unfreezes and archives every room, requires the healthy half of 32 signed clients to observe 4409 plus denied archived-room reconnects, reopens all rooms, then resumes the stale replica and requires its half to fence with 1012. Every client must establish a second activated connection, reconcile authoritative paginated history, converge, and drain its database lease. The profile records exact client-path counts and recovery time; it is bounded shared-runner evidence, not a device-network or capacity guarantee.
- A reusable, manually dispatchable Kubernetes preview acceptance workflow creates a pinned kind/Kubernetes cluster, builds and imports the reviewed application image, and provisions a digest-pinned disposable PostgreSQL 18.6 server with a one-run CA and hostname-verified TLS. It applies the checked two-replica StatefulSet, requires the exact `samsarix-chat-0`/`samsarix-chat-1` live database identities, proves cross-replica HTTP and signed-member WebSocket delivery, replaces one same-version Pod, and verifies stable identity plus retained state. Its smoke client rejects non-loopback targets. This is single-node evaluation evidence, not external database failover/fencing, ingress, NetworkPolicy, capacity, registry provenance, or owner-environment acceptance.
...
- PostgreSQL rate-limit bucket creation now releases optimistic bucket-row locks before acquiring the global capacity lock. This keeps request and maintenance lock ordering consistent and prevents window-boundary deadlocks under sustained new-identity traffic; hot existing buckets retain their row-local fast path.
