...
- Corrected webhook timeout/shutdown guidance: the existing timeout covers blocking socket operations, not total request duration or lifespan cleanup. A finite supervisor termination deadline is needed; slow-header/DNS/TLS and bounded in-process delivery/shutdown remain a P1 hardening gate.
