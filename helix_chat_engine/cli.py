...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="helix-chat",
        description="Run the local-first Helix room chat service.",
    )
    ...
    serve.add_argument("--database", type=Path, help="SQLite path (overrides HELIX_CHAT_DATABASE)")
    serve.add_argument(
        "--allow-insecure-public",
        action="store_true",
        help="allow a non-loopback bind without HELIX_CHAT_API_KEY (unsafe)",
    )
    serve.add_argument(
        "--log-level",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not _is_loopback_host(args.host) and settings.api_key is None and not args.allow_insecure_public:
        parser.error(
            "refusing an unauthenticated public bind; set HELIX_CHAT_API_KEY or explicitly pass --allow-insecure-public"
        )
