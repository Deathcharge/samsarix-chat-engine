"""Runtime configuration for Helix Chat Engine."""
...
def _read_origins() -> tuple[str, ...]:
    raw = os.getenv("HELIX_CHAT_ALLOWED_ORIGINS", "")
...
class Settings:
    ...
    database_path: Path = Path("data/helix-chat.db")
    ...
    def __post_init__(self) -> None:
        if self.api_key is not None and not 16 <= len(self.api_key) <= 4_096:
            raise ConfigurationError("HELIX_CHAT_API_KEY must be between 16 and 4096 characters")
        checks = {
    ...
    def from_env(cls) -> Settings:
        """Load settings from documented ``HELIX_CHAT_*`` environment variables."""

        api_key = os.getenv("HELIX_CHAT_API_KEY") or None
        return cls(
            database_path=Path(os.getenv("HELIX_CHAT_DATABASE", "data/helix-chat.db")),
            api_key=api_key,
            allowed_origins=_read_origins(),
            max_message_chars=_read_int("HELIX_CHAT_MAX_MESSAGE_CHARS", 4_000, minimum=1, maximum=100_000),
            max_connections=_read_int("HELIX_CHAT_MAX_CONNECTIONS", 200, minimum=1, maximum=100_000),
            max_connections_per_room=_read_int("HELIX_CHAT_MAX_CONNECTIONS_PER_ROOM", 100, minimum=1, maximum=100_000),
            messages_per_minute=_read_int("HELIX_CHAT_MESSAGES_PER_MINUTE", 60, minimum=1, maximum=100_000),
            max_rooms=_read_int("HELIX_CHAT_MAX_ROOMS", 1_000, minimum=1, maximum=1_000_000),
            max_stored_messages=_read_int("HELIX_CHAT_MAX_STORED_MESSAGES", 100_000, minimum=1, maximum=10_000_000),
            max_stored_messages_per_room=_read_int(
                "HELIX_CHAT_MAX_STORED_MESSAGES_PER_ROOM", 10_000, minimum=1, maximum=1_000_000
            ),
            websocket_auth_timeout_seconds=_read_float("HELIX_CHAT_WS_AUTH_TIMEOUT", 5.0, minimum=0.1, maximum=60),
            websocket_send_timeout_seconds=_read_float("HELIX_CHAT_WS_SEND_TIMEOUT", 2.0, minimum=0.1, maximum=60),
            websocket_max_bytes=_read_int("HELIX_CHAT_WS_MAX_BYTES", 16_384, minimum=256, maximum=16_777_216),
        )
