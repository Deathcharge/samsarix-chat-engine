...
- Replace stale client state from reconnect history and apply buffered/live mutations through the SDK's synchronization-aware, bounded room timeline.
...
- Target up to ten host-resolved subjects for highlighting, escalation, or host-owned notifications without granting access or adding a push provider.
...
Mention targets are explicit stable host subjects, not parsed `@names` or authorization:

```json
{"type":"message","content":"Please review","mentioned_subjects":["agent-7","billing-oncall"]}
```
...
Clients receive `message.created`, `message.updated`, `message.deleted`, `message.reaction.updated`, `message.pin.updated`, `typing.started`, `typing.stopped`, room-state, moderation, presence, `pong`, and structured `error` events. Browser clients first receive `auth.required` and reply with `{"type":"auth","token":"..."}`. Token identity supplies the username; legacy local/operator connections still use `?username=`. API keys and tokens are never accepted in query strings.
...
| `SAMSARIX_CHAT_WEBHOOK_EVENTS` | all six when URL set | Comma-separated selected committed message/moderation/reaction/pin event types |
...
- During initialization, room broadcasts are buffered after registration and flushed after `ready`/`history`, before live delivery starts. Defaults are 64 events / 256 KiB per pending socket and 8 MiB shared serialized payload budget, including in-flight flushes. Overflow or a flush exceeding one send-timeout interval closes 1013 for reconnect/history recovery. History and buffered events can overlap; the SDK timeline replaces from history, upserts complete messages by ID, and evicts oldest entries beyond its configured bound. See [the initialization contract](docs/API_REFERENCE.md#server-events).
- Servers advertise `snapshot_sync_v1` in `ready.capabilities`. A capable client sends `{"type":"sync"}` after history and treats `sync.completed` as the local initialization-buffer boundary. This is a snapshot handoff, not a durable event cursor, exactly-once delivery, or proof that another PostgreSQL replica has no relay lag.
- PostgreSQL admission records the committed join-event sequence. A delayed relay skips earlier events for that new socket, preventing an old archive or ban from closing a later authorized reconnect. Events after admission still apply, including to older sockets; the internal sequence is not exposed as a durable per-client replay cursor.
