...
async def _reset_postgres_test_database(conninfo: str) -> None:
    ...
    async with await psycopg.AsyncConnection.connect(conninfo, autocommit=True) as connection:
        ...
        await connection.execute("DROP TABLE IF EXISTS public.samsarix_typing_states")
        ...
        await connection.execute("DROP TABLE IF EXISTS public.samsarix_realtime_retention")
