...
Line 1 has `type: samsarix.room_export`, `schema_version: 8`, an export timestamp, and room metadata. Each later line has `type: message` and one complete current message or tombstone. Export schema 8 retains nullable `parent_message_id`, sorted grouped `reactions`, nullable `pinned_at`/`pinned_by`, bounded application `metadata`, and ordered host-owned `attachments`, then adds ordered host-resolved `mentioned_subjects`; readers should reject unknown major schema values. The audit action is `room.export_requested`: it records that the export request was accepted, not that the client received every byte.
...
Graceful webhook shutdown stops new claims, cancels the caller's wait and interrupts the owned TCP/TLS socket. `SAMSARIX_CHAT_WEBHOOK_TIMEOUT` is a total network-attempt budget (10 seconds by default, 30 maximum), including slow response headers and the caller's DNS wait. Native DNS itself cannot be forcibly stopped: one bounded daemon job may remain until resolution returns, without further claims, late network sends, or blocking interpreter exit. A permanently stuck resolver requires diagnosis/restart; watch pending backlog rather than treating ordinary service readiness as proof of webhook progress. See the precise resource/retention limits in [webhook recovery](WEBHOOKS.md#retry-and-recovery).

Storage claim/outcome operations and other application cleanup retain separate budgets. Continue to configure a process supervisor with a finite forced-termination deadline; a Uvicorn request-drain timeout alone is not a hard bound on all lifespan cleanup. Cancellation, stop or forced termination may leave an unrecorded attempt: SQLite retries the pending row on restart, while PostgreSQL allows another worker to reclaim it after its 60-second database lease expires. A network timeout is instead recorded as a retryable failure when storage remains available. Neither closing a socket nor timing out can undo receiver-side effects; receivers must deduplicate the stable delivery ID.
...
These CLI procedures apply to the supported SQLite deployment. The guarded PostgreSQL preview deliberately rejects SQLite backup and restore commands; use PostgreSQL-native, encrypted backups and point-in-time recovery instead, as described in the [PostgreSQL preview runbook](POSTGRES_PREVIEW.md#migration-backup-and-rollback).
...
The unreleased development build advances SQLite through schema 6 for the indexed nullable thread parent, schema 7 for actor-unique message reactions plus transactionally maintained grouped summaries, schema 8 for indexed shared pin metadata, schema 9 for bounded application message metadata, schema 10 for bounded host-owned attachment references, and schema 11 for bounded host-resolved mentions. Existing messages remain top level with no reactions, pins, application metadata, attachments, or mentions. Take a verified backup before first opening a database with this build; released binaries that understand only schema 5 will refuse the upgraded database, so rollback requires restoring the matching pre-upgrade backup.
...
The cross-room read-state query is also read-only: it does not create cursors, audit events, exports, or message copies. It evaluates at most 100 host-supplied room IDs in one statement and exposes only cursor/count plus latest visible message ID/time. `SAMSARIX_CHAT_READ_STATE_QUERIES_PER_MINUTE` bounds each signed subject's query rate. SQLite enforces that budget in its one supported process; PostgreSQL coordinates it across replicas. Hosts remain responsible for paging their membership/assignment records and must not treat the endpoint as room discovery.
