...
Graceful shutdown waits for an in-flight webhook attempt. `SAMSARIX_CHAT_WEBHOOK_TIMEOUT` limits individual blocking socket operations (10 seconds by default, 30 maximum), not the whole delivery, DNS resolution or application lifespan shutdown. Slow response headers can keep an attempt running beyond that setting. Configure a process supervisor with a finite forced-termination deadline; a Uvicorn request-drain timeout alone is not a hard bound on this lifespan cleanup. A true in-process delivery/shutdown deadline remains a documented P1. Forced termination may leave an unrecorded attempt: SQLite retries the pending row on restart, while PostgreSQL allows another worker to reclaim it after its 60-second database lease expires. Receivers must deduplicate the stable delivery ID.
