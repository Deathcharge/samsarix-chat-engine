...
Line 1 has `type: samsarix.room_export`, `schema_version: 7`, an export timestamp, and room metadata. Each later line has `type: message` and one complete current message or tombstone. Export schema 7 retains nullable `parent_message_id`, sorted grouped `reactions`, nullable `pinned_at`/`pinned_by`, and bounded application `metadata`, then adds ordered host-owned `attachments`; readers should reject unknown major schema values. The audit action is `room.export_requested`: it records that the export request was accepted, not that the client received every byte.
...
The unreleased development build advances SQLite through schema 6 for the indexed nullable thread parent, schema 7 for actor-unique message reactions plus transactionally maintained grouped summaries, schema 8 for indexed shared pin metadata, schema 9 for bounded application message metadata, and schema 10 for bounded host-owned attachment references. Existing messages remain top level with no reactions, pins, application metadata, or attachments. Take a verified backup before first opening a database with this build; released binaries that understand only schema 5 will refuse the upgraded database, so rollback requires restoring the matching pre-upgrade backup.
