...
Local `python -m pytest -q tests/test_postgres_schema_startup.py tests/test_postgres_deadline.py --timeout=20 --tb=short` passes **28 tests**. Ruff lint/format, mypy and `git diff --check` pass. Nine live cases cover concurrent application-table access locks, server-enforced read-only inspection, unchanged metadata/retention rows across starts, blocked migration rollback and retry, advisory-lock waiting with a concurrent version change, actual pool cleanup after cancelled lock waiting, and an existing WebSocket across three replica starts. Historical v2/v6 migration and concurrent empty-schema tests remain required gates. These are controlled PostgreSQL/ASGI tests, not a load benchmark or proof of online schema upgrades. No local PostgreSQL/Docker executable is available; full local and live CI, final package evidence and merge results will be recorded on the PR.
