...
Latest verified baseline: clean `main...origin/main` at `f62fabf` (merged PR #26), with no open PRs. The exact pre-merge file tree passed all ten CI jobs, 212 tests and 88.38% branch-inclusive coverage, and the post-merge main run also passed. All four late review findings from PR #25 were addressed by PR #26; the earlier increment evidence below remains historical.

PR #27 adds a separate checked-out PostgreSQL operation deadline, finite replacement-connect timeout, and per-session statement/idle-transaction limits. Deadline expiry closes the stalled session before interrupting its waiter, so Psycopg cancellation cannot wait indefinitely on the same transport. The fault proxy now verifies silent bidirectional application-traffic stalls without cutting TCP, alongside reset/refusal, healthy-peer progress, readiness fencing, explicit reconnect/history recovery, and resumed delivery. A live query test proves a timed-out session is discarded and the single-connection pool recovers. The Python/OS matrix now installs the PostgreSQL extra so database-independent deadline tests run throughout the compatibility matrix; Python 3.10 explicitly skips overlapping-cancellation discrimination because it lacks cancellation-count APIs.

The first PR #27 CI run at `d798801` passed all jobs, 231 tests and 88.41% coverage, including both real-network failure modes and the live query-deadline test. After adding server limits and startup-timeout cleanup coverage, local `python -m pytest -q -m "not postgres" --timeout=60` passed **174 tests**, with 59 deselected; the focused deadline/configuration suite passed 40 tests. Ruff lint/format, mypy and diff checks pass. The final server-limit/documentation head must pass CI before merge. No local PostgreSQL or Docker executable is available, so live tests run against the dedicated CI database. No schema change (PostgreSQL 8 / SQLite 5), new dependency, production deployment, publication, or paid resource is involved.

Timeouts do not prove rollback after a lost commit reply. The [preview operations guide](POSTGRES_PREVIEW.md#operation-deadlines-and-ambiguous-outcomes) records safe idempotent retry/reconciliation, separate pool and operation limits, the test's accelerated timing, and the distinction between a traffic-stalling proxy and kernel-level packet loss. Remaining release gates include kernel packet-loss/failover testing, live-lag fencing, cross-process moderation/quotas/webhooks, sustained load/reconnect storms, deployment manifests, and verified PostgreSQL-native backup/PITR/rollback.
