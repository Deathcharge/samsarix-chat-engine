...
Latest verified baseline: clean `main...origin/main` at `eead4bc` (merged PR #27), with no open PRs at the start of this increment. Its main CI run passed all ten jobs, 233 tests and 88.45% branch-inclusive coverage. Earlier increment evidence below remains historical.
...
Adversarial review leaves explicit P1 follow-up work: cover cleanup when admission succeeds but initial room/history queries fail or are cancelled, and exercise archive versus lease-renewal ordering (renewal currently reports an unavailable lease generically). These are not proven fixed by normal-path moderation tests. The pending-socket ready/history activation window also has no buffered replay guarantee; clients must reload authoritative history. Remaining v0.13 gates include live-lag fencing, cross-process quota contention and webhook-worker crash recovery, kernel packet-loss/failover testing, sustained load/reconnect storms, deployment manifests, and verified PostgreSQL-native backup/PITR/rollback. See the [preview guide](POSTGRES_PREVIEW.md) and [acceptance checklist](MULTI_INSTANCE_ARCHITECTURE.md#release-acceptance-gates).
...
- [ ] Cover initial-handshake failure/cancellation cleanup and archive versus lease-renewal ordering; do not infer these guarantees from normal-path moderation acceptance.
