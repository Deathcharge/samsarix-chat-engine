...
- Version 2 records every dropped slot's bounded synthetic index, reason, phase, relative observation time and lateness. The hard 20000-cycle profile bound also bounds this list. It never records an absolute monotonic value, operation content or credential.
...
`count` waits for the real unread backlog to exceed the configured count bound while the lease remains live. Its injected bound is the maximum of 100, five seconds of scheduled create cycles, and two presence events per configured subscriber. This admits the profile's own bounded leave/rejoin population before intentionally pausing a replica; it does not change the product default or hide drops, refusals, fencing, missing events or failed convergence. `age` waits for real database-clock age to exceed three seconds while the lease remains live. `retained-gap` uses natural three-second lease expiry and waits for the healthy application's actual periodic pruning to pass the old cursor. No clock, event, cursor, lease or pruning timer is rewritten. Fault profiles require at least 180 scheduled seconds to include recovery and a subsequent live interval.
...
`schema_version: 2` contains the profile, effective non-secret server overrides, exact checkout revision, Python/OS/CPU information, PostgreSQL version, counts, exact nearest-rank p50/p95/p99/max latency distributions, timeline, per-client counters and one-second-interval samples. Empty distributions have null percentiles, not zeroes. Version 2 adds completion-time ten-second latency windows and the diagnostics below; earlier accepted reports remain version 1 evidence rather than being rewritten.

Samples include actual unread event count/age, live lease/cursor/retention positions, application/driver cumulative CPU and I/O counters from procfs, application and driver RSS/PSS from the kernel's documented [`smaps_rollup`](https://www.kernel.org/doc/html/latest/filesystems/proc.html), host-wide CPU/I/O/memory pressure, bounded PostgreSQL application-state/wait categories and cumulative database statistics, total scratch database bytes, current harness counters, and the diagnostic sample's own duration. Sampling and SQL instrumentation share the same host and add overhead. These are sampled values, not hard maxima or memory-leak proof; RSS can double-count shared pages, PostgreSQL memory remains excluded, procfs I/O counters do not all represent physical storage, and cumulative counters require within-report deltas. PostgreSQL observers use autocommit to avoid holding a stale transaction snapshot; statistics can lag and active work may not yet be reflected, as its [statistics documentation](https://www.postgresql.org/docs/current/monitoring-stats.html) explains.

Host pressure uses Linux [Pressure Stall Information](https://www.kernel.org/doc/html/latest/accounting/psi.html). The ten-second latency windows group observations by completion time and intentionally retain request-start-to-frame semantics; setup and drain buckets can contain only connection/history timings. PostgreSQL waits are instantaneous samples, not wait-duration totals, and do not include query text, users, client addresses or hostnames. Per-process `/proc/<pid>/stat` CPU values and `/proc/<pid>/io` counters are cumulative. Server warning evidence contains counts for four static fencing labels from a bounded log tail, never raw log lines; a zero means “not in the retained tail,” not “never happened.” Unsupported or inaccessible PSI is null, not a false zero.
...
PR #41 subsequently merged as `79d3afd` after [final run 33424183292](https://github.com/Deathcharge/samsarix-chat-engine/actions/runs/33424183292) passed all 14 jobs and all four profiles on PostgreSQL 18.6. Its [acceptance record](https://github.com/Deathcharge/samsarix-chat-engine/pull/41#issuecomment-5482649644) preserves exact final-head report digests and package evidence. The final steady run's POST p50/p95/p99/max was 33.36/279.58/636.56/999.31 ms and live-delivery p99 approximately 761–762 ms, despite zero dropped cycles and complete event/state convergence. Fault-run POST p99 was 14.00–15.81 ms; recovery after resumption was 0.629–0.850 s. The difference from the earlier results below has no established cause: these different shared-runner executions prove neither a patch regression nor harmless runner noise. Controlled-host latency diagnosis and longer profiles remain open; do not select only the faster result for a capacity claim.
