...
Controlled SQLite and PostgreSQL application tests pause a captured history snapshot, commit create/edit/delete operations through HTTP, wait for dispatch, and then verify that initial history plus queued mutations converges to current database rows, including tombstones. This is real-storage/ASGI evidence, not a network-process stall, load, or failover benchmark. Post-admission relay events can overlap history and duplicates remain possible. `snapshot_sync_v1` identifies when this process has drained the local initialization buffer, but adds no durable per-client cursor or proof that another replica has reached the global event head. Presence counts remain event-time snapshots and can predate the later count in `ready`. The separate measured workload covers one combined process-pause/lifecycle/reconnect case; broader outage timing and sustained soak remain release gates.

## Stable replica identity and Kubernetes evaluation

The checked [Kubernetes PostgreSQL preview](../deploy/kubernetes/README.md) runs two application Pods in a `StatefulSet`. `SAMSARIX_CHAT_POSTGRES_INSTANCE_ID` comes from the Downward API's `metadata.name`, giving each ordinal Pod a stable value such as `samsarix-chat-0`. A repository verifier fails on a literal/shared identity, inline database URL, selector drift, missing secret projection, weakened container security, missing probes/resources, or a non-headless identity service.
...
Startup takes a transaction-scoped advisory migration lock and advances the internal PostgreSQL schema to version 14. Version 9 adds the nullable, indexed message-parent reference used by one-depth replies; version 10 adds actor-unique reaction rows and transactionally maintained JSON summaries; version 11 adds nullable shared-pin metadata plus a partial room/pin-time index; version 12 adds bounded JSON application message metadata; version 13 adds bounded host-owned attachment references; and version 14 adds bounded host-resolved mentions. Existing messages remain top level with no reactions, pins, application metadata, attachments, or mentions. Newer unknown schemas fail closed. Before the first preview startup or upgrade, take a provider-native physical/PITR backup or a tested logical backup that includes all `public.samsarix_*` tables and identity sequences.

Message deletion and physical retention scrub content, reaction summaries, and pin metadata from retained message-event payloads before the authoritative row disappears. Reaction and pin events additionally replace their action actor with `[deleted]`, so the durable event log does not preserve those identities after the message is gone. Already-delivered or concurrently in-flight events remain receiver responsibilities.
...
- Two-process normal delivery, kill/lease-expiry/restart, kernel-paused count/age lag, natural retained-gap recovery, a bounded 32-client reconnect storm, database TCP reset/refusal, and silent bidirectional database-traffic stalls with explicit reconnect/history recovery run in CI. Kernel-level packet blackholes, database failover, controlled-host capacity, device/network diversity, and sustained load/soak evidence remain pending. If a notification listener is introduced, its loss/reconnect behavior will require additional tests; polling is the current implementation.
- Live-lag checks fence before the next over-limit batch; in-flight dispatch, interrupted storage, and lease expiry can still delay retention. Hard disk/latency bounds and reconnect-storm capacity beyond the exact published profile are not claimed.
